...
import { GLOB_ASTRO, GLOB_MARKDOWN, GLOB_TS, GLOB_TSX, GLOB_TYPES } from '../constants';
 ...
 */
export function createTypeScriptConfig(options: ConfigTypeScriptOptions = {}): TypedConfigItem[] {
  ...
  function createParserConfig(
    ...
    const parserConfig: TypedConfigItem = {
      name: `sankeyangshu/ts/${enableTypeAware ? 'parser-type-aware' : 'parser'}`,
      files,
      ignores: [...ignores],
      languageOptions: {
  ...
  return [
    ...
    ...(enableTypeAwareLint
      ? [createParserConfig(false, files), createParserConfig(true, filesTypeAware, ignoresTypeAware)]
      : [createParserConfig(false, files)]),

    {
      name: 'sankeyangshu/ts/rules',
      files,
      rules: {
        ...recommendedRules,
    ...
    ...(enableTypeAwareLint
      ? ([
          {
            name: 'sankeyangshu/ts/rules/type-aware',
